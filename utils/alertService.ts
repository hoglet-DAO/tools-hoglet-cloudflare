...
const createExplorerLink = (txHash: string, network: string, t: (key: string) => string) => {
    const explorerUrl = getExplorerUrl(network, "transaction", txHash);
    if (!explorerUrl) return '';
    return `<a href="${explorerUrl}" target="_blank" rel="noopener noreferrer" class="text-cyan-400 hover:underline">${t('viewOnExplorer')}</a>`;
};

export const showStakeSuccessAlert = (txHash: string, network: string, t: (key: string) => string) => {
    const htmlContent = createExplorerLink(txHash, network, t);
    return showSuccessAlert(t('stakeSuccessTitle'), htmlContent);
};

export const showUnstakeSuccessAlert = (txHash: string, network: string, t: (key: string) => string) => {
    const htmlContent = createExplorerLink(txHash, network, t);
    return showSuccessAlert(t('unstakeSuccessTitle'), htmlContent);
};

export const showEmergencyUnstakeSuccessAlert = (txHash: string, network: string, t: (key: string) => string) => {
    const htmlContent = createExplorerLink(txHash, network, t);
    return showSuccessAlert(t('emergencyUnstakeSuccessTitle'), htmlContent);
};
